...
from xmco_modules.models import YunoAdvisoryConnectorConfiguration

SEVERITY_ORDER = ["low", "medium", "high", "urgent"]
BATCH_SIZE = 50


class YunoAdvisoryConnector(Connector):
    module: XmcoModule
    configuration: YunoAdvisoryConnectorConfiguration

    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self._advisory_context = PersistentJSON("advisory_context.json", self._data_path)
    ...
    def _get_cursor(self) -> Optional[str]:
        with self._advisory_context as cache:
            cursor = cache.get("last_advisory_cursor")
            if cursor:
                return cursor
    ...
    def _set_cursor(self, cursor: str) -> None:
        with self._advisory_context as cache:
            cache["last_advisory_cursor"] = cursor

    def _build_where(self, cursor: Optional[str]) -> Optional[dict]:
        where: dict = {}
        if cursor:
            where["_created"] = {"$gt": cursor}
        if self.configuration.min_severity:
            try:
                idx = SEVERITY_ORDER.index(self.configuration.min_severity)
                where["severity"] = {"$in": SEVERITY_ORDER[idx:]}
            except ValueError:
                pass
    ...
    def fetch_events(self) -> Tuple[List[str], Optional[str]]:
        ...
        while True:
            response = self.client.get_advisories(page=page, max_results=BATCH_SIZE, where=where)
    ...
    def run(self) -> None:
        self.log(message="Starting XMCO Yuno Advisory connector", level="info")
        while self.running:
            try:
                ...
                if events:
                    ...
                    self.log(message=f"Pushed {len(events)} advisory events to intake", level="info")
                else:
                    self.log(message="No new advisory events found", level="info")
            except Exception as e:
                self.log_exception(e, message="Error while fetching advisory events")
